...
void D_EXP_Bracket_ProcessBracket(M_EXP *fam_exp, const string code, const string word, M_Cursor *cursor);
...
void D_EXP_Bracket_Process(M_EXP *fam_exp, const string code, const string word, M_Cursor *cursor) {
    ...
    if (status == NFA_EXPression_Status_Bracket) {
        D_EXP_Bracket_ProcessBracket(fam_exp, code, word, cursor);
    }
}